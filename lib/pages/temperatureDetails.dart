          ...
          children: <Widget>[
            FutureBuilder(
              ...
              builder: (BuildContext context, AsyncSnapshot snapshot) {
                if (snapshot.hasData) {
                  return LineGraphCard(temperatures: snapshot.data, isZoomable: true);
                } else {
                  return CustomCard(
                    child: Container(height: 150, child: Center(child: CircularProgressIndicator())),
                  );
                }
              },
            ),
            buildDetailRow(),
            CustomCard(margin: EdgeInsets.only(left: 20, right: 20, top: 10), child: Text("das ist ein text")),
          ],
        ),
      ),
    );
              ...
              builder: (BuildContext context, AsyncSnapshot snapshot) {
                if (snapshot.hasData) {
                  ...
                  return Column(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: <Widget>[
                      Text("${t.temperature}", style: TextStyle(fontSize: 25, fontWeight: FontWeight.bold)),
                      Text("Highest Temperature", style: TextStyle(color: Colors.grey)),
                    ],
                  );
                } else {
                  return Center(
              ...
              builder: (BuildContext context, AsyncSnapshot snapshot) {
                if (snapshot.hasData) {
                  ...
                  return Column(
                    ...
                    children: <Widget>[
                      Text("${t.temperature}", style: TextStyle(fontSize: 25, fontWeight: FontWeight.bold)),
                      Text("Lowest Temperature", style: TextStyle(color: Colors.grey)),
                    ],
                  );
                } else {
                  return Center(
